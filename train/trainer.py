...
def main():
    ...
    # Data
    monet_ds = ImageDataset(
        data_path=args["images_path"], center_crop=args["center_crop"]
    )
    monet_dl = DataLoader(
        dataset=monet_ds, batch_size=args["batch_size"], shuffle=True, num_workers=8
    )

    # Logger
    logger = WandbLogger(project="monet")
